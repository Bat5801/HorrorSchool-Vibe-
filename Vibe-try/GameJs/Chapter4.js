class Chapter4 {
    ...
    showResultScreen() {
        ...
        // 显示章节名称和通关时间
        this.game.elements.resultChapter.textContent = '第四章 - 「黑暗侵蚀」';
        ...
        this.game.elements.nextChapterBtn.textContent = '继续第五章';

        // 检查第五章是否已解锁，如果未解锁则禁用按钮
        if (!this.game.isChapterUnlocked('chapter5')) {
            this.game.elements.nextChapterBtn.disabled = true;
            this.game.elements.nextChapterBtn.textContent = '继续第五章 (未解锁)';
        } else {
            this.game.elements.nextChapterBtn.disabled = false;
        }

        // 绑定返回主菜单按钮事件（如果需要）
        if (!this.backToMenuBound) {
            this.game.elements.backToMenuBtn.addEventListener('click', () => {
                this.game.returnToMainMenu();
            });
            this.backToMenuBound = true;
        }
    }
}
